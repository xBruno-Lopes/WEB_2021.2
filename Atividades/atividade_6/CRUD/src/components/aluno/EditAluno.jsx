...
export default class EditAluno extends Component {
    ...
    componentDidMount(){
        axios.get('http://localhost:3001/estudantes/retrieve/'+this.props.match.params.id) 
        .then((res)=>{
            this.setState({
                nome:res.data.nome,
    ...
    onSubmit(e){
        ...
        const estudanteEditado = {nome:this.state.nome, 
                                  ...
                                  IRA:this.state.IRA}
        axios.put('http://localhost:3001/estudantes/update/'+this.props.match.params.id, estudanteEditado)
        .then(
            (res)=>{
