...
export default props=>{
    ...
    return(
        <div>
            ...
            <h2>Total de Votos: {total}</h2>
            <button onClick={()=>[setFort(fort+1), setTotal(total+1)]}> Votar em Fortaleza </button>
            <button onClick={()=>[setQuix(quix+1), setTotal(total+1)]}> Votar em Quixadá </button>
            <button onClick={()=>[setOcar(ocar+1), setTotal(total+1)]}> Votar em Ocara </button>
            <button onClick={()=>[setLimo(limo+1), setTotal(total+1)]}> Votar em Limoeiro </button>
        </div>
    )
}